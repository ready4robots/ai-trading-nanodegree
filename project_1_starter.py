...
def portfolio_returns(df_long, df_short, lookahead_returns, n_stocks):
    ...
    """
    df_long_weights = df_long/n_stocks 
    df_short_weights = (df_short/n_stocks) * -1 
    
    return (df_long_weights + df_short_weights) * lookahead_returns 
...
def analyze_alpha(expected_portfolio_returns_by_date):
    ...
    n, (smin, smax), sm, sv, ss, sk = stats.describe(expected_portfolio_returns_by_date)
    
    t_stat, p_value = stats.ttest_1samp(expected_portfolio_returns_by_date, m)
...
# P value (.07) is greater than our alpha (.05); we therefore cannot reject the null hypothesis that the actual mean from the signal is zero.
